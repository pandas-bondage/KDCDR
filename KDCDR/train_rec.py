...
from KDCDR.utils.loader import DataLoader
from KDCDR.utils.GraphMaker import GraphMaker ,normalize,sparse_mx_to_torch_sparse_tensor
from KDCDR.utils import torch_utils, helper
import json
import codecs
import copy
...
parser.add_argument('--dataset', type=str, default='sport_phone', help='')
# model part
parser.add_argument('--model', type=str, default="KDCDR", help="The model name.")
...
parser.add_argument('--tau', type=float, default=45)
# train part
...
if "KDCDR" in opt["model"]:
    ...
    A_B_graph = "../dataset/" + filename + "/A_B_train.txt"
    A_B_G_nonenormal = GraphMaker(opt, A_B_graph)
    A_B_G_nonenormal_UV = A_B_G_nonenormal.UV
    A_B_G_nonenormal_VU = A_B_G_nonenormal.VU
...
if opt["cuda"]:
    ...
    A_B_G_nonenormal_UV = A_B_G_nonenormal_UV.cuda()
    A_B_G_nonenormal_VU = A_B_G_nonenormal_VU.cuda()
...
source_UU_adj, source_VV_adj = source_UV @ source_VU, source_VU @ source_UV
target_UU_adj, target_VV_adj =target_UV@target_VU,target_VU@target_UV
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    for i, batch in enumerate(train_batch):
        global_step += 1
        loss = trainer.reconstruct_graph(batch,A_B_G_nonenormal_UV,A_B_G_nonenormal_VU,source_UU_adj,source_VV_adj,target_UU_adj,target_VV_adj, source_UV, source_VU, target_UV, target_VU,None,None, None,None,None,None,epoch)
        train_loss += loss
    ...
    trainer.evaluate_embedding(A_B_G_nonenormal_UV, A_B_G_nonenormal_VU, source_UU_adj, source_VV_adj, target_UU_adj,target_VV_adj, source_UV, source_VU, target_UV, target_VU)
    NDCG = 0.0
    HT = 0.0
    valid_entity = 0.0
    ...
    if epoch == 100:
        print('source_hit:{},source_ndcg:{},target_hit:{},target_ndcg:{}'.format(s_BEST_HIT, s_BEST_NDCG, t_BEST_HIT,
                                                                                 t_BEST_NDCG))
    # save
